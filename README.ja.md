...
> ⚠️ **ロール分離は未実装**です。Access を通過した人は誰でもコンソールを開き、全症例を閲覧できます。ハッシュルート(`#/report`)は Access のパスルールでは分離できません——`#` 以降はサーバーに送信されないためです——アプリケーション層でのロール管理が必要です。それまでは、リスト上の全員を PV 担当者とみなしてください。
