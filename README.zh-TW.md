...
*   🗂️ **藥安收案後台（「通報收案」頁籤）**
    ...
    *   全流程留**稽核軌跡**（誰、何時、做了什麼）。
    *   **追蹤報告**：補件收到回覆後可由原案一鍵建立（編號 `原案-F1`），Day 0 設為**獲知新資訊之日**；帶來重要新資訊者 15 日時鐘重新起算，否則不重啟並收錄於定期安全性報告。追蹤鏈自動排除於重複個案偵測之外。
    *   **境外個案**：通報端可選反應發生國別（CIOMS 1a），後台標記境外個案並印入 CIOMS 與 E2B `E.i.9`。
...
業務首次登入會先填一次**通報者基本資料**（姓名、員編、電話、公司——CIOMS 26／24a），之後每份通報自動帶入，表單第一屏從六個輸入框收成一張摘要卡。可寫欄位是明確的白名單，沒有人能靠編輯自己的檔案把自己變成藥安人員。
...
**角色**記在 D1，因為 Access 只回答「這個人是不是自己人」，不回答「這個人該看到什麼」：
...
不在 `ae_users` 裡的一律是 `rep`——設定漏了的後果是「某人看不到全部個案」（他會來反映），而不是「某人看得到全部個案」（沒人會來反映）。第一位藥安人員用 `wrangler secret put AE_PV_EMAILS` 開機。

> ⚠️ 執行點在 Worker，不在畫面。hash 路由（`#/report`）無法用 Access 的路徑規則分權——`#` 後的片段不會送到伺服器——所以前端的角色判斷只是體驗；每一條 API 都自己查角色，且清單過濾寫在 SQL 而非 JS。
