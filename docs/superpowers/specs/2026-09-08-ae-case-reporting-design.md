...
目前**任何通過 Access 的人都能開啟後台並讀取全部個案**，
`GET /api/ae-reports` 也對任何已驗證身分回傳完整清單。
與使用者原始意圖「我只想讓業務通報」之間存在落差。

hash 路由（`#/report`）**無法**用 Access 的路徑規則分權：`#` 後的片段依 HTTP 規範
不會送到伺服器，Cloudflare 看不到它。要分權只能在應用層做：

1. D1 開一張 email → 角色（`rep` / `pv`）對照表
2. Worker 依角色決定可用的動作：`rep` 只能 `POST`（新增個案），
   `pv` 才能 `GET` 清單、`PATCH`、`DELETE`
3. 前端依角色隱藏後台頁籤（僅為體驗，真正的守門在 Worker）

⚠️ 在這件事完成之前，**允許名單上的每一個人都等於藥安人員**，名單須以此為前提決定。
...
- ⚠️ 尚無角色分權：通過 Access 的業務也讀得到全部個案（見 §7A.6）
- 附件未做自動去識別化（見 §9）
