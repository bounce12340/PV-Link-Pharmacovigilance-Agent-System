...
### 7A.5 身分驗證：Cloudflare Access（公司信箱 Email OTP）

業務端不自建帳號密碼——多一套密碼就是多一組會外洩、會被共用、要負責重設的憑證。
改用 **Cloudflare Access 的 Email OTP**：使用者輸入**公司信箱**，收一次性代碼，即完成登入。

**為什麼限定公司信箱**：離職即失效是自動的。信箱在離職當天由 IT 停用，
停用後收不到 OTP，Access policy 忘了清也進不來——安全性不依賴任何人記得去做某件事。
私人 Gmail 沒有這個性質：離職者的信箱誰也停不掉。

**允許名單用個別 email 逐一列舉，不用網域規則。**
`Emails ending in @公司網域` 的意思是「全公司每個人都能讀病人不良反應個案」，
含財會、人資、工讀生。個案是《個資法》第 6 條特種個資，存取範圍應是
「業務 + 藥安人員」而非「有公司信箱的人」。人數多到難以維護時用 Access Group，
不是退回網域規則。
...
後者由應用層決定，而且**不能靠 hash 路由**：`#/report` 的片段依 HTTP 規範
...
| 角色 | 能做的事 |
|---|---|
| `rep`（業務） | 新增個案；**只讀得到自己送的個案** |
| `pv`（藥安人員） | 讀寫全部個案：判定、編碼、追蹤報告、軟刪除 |

#### 預設值選在哪一邊

`ae_users` 查無此人時一律視為 `rep`。這是刻意的：設定漏了的後果是
「某人看不到全部個案」（他會來反映），而不是「某人看得到全部個案」（沒人會來反映）。
`normalizeRole()` 也把任何無法辨識的值——`null`、空字串、拼錯的字串——一律降級為 `rep`。

#### 開機死結

`ae_users` 一開始是空的，若只認資料表，第一個藥安人員永遠設不進去：
沒有人有權限去新增第一個有權限的人。因此保留環境變數 `AE_PV_EMAILS`
（逗號分隔）作為 bootstrap，且應以 `wrangler secret put` 設定——那是真實人員信箱，
不該進 git。

#### 執行點在 Worker，不在前端

前端的角色判斷（`components/Root.tsx` 向 `/api/me` 問角色，`rep` 一律導向通報表單）
**只是體驗**。任何人都能直接打 API，所以每一條路由都自己查角色：

- 列表過濾寫在 **SQL 的 `WHERE`** 裡，不是取出全部再於 JS 篩掉——
  後者只要有人改了迴圈就整份外洩，而且真的把別人的病人資料讀進了記憶體。
- 讀不到別人的個案時回 **404 而非 403**：403 等於告訴對方「這個 id 存在」，
  個案編號是可猜的序號，這點差別足以讓人推敲出通報量。
- 附件的權限**跟著個案走**：先確認讀得到該個案，才談附件。

#### rep 重送自己的個案

`POST` 對 `rep` 不是無條件放行。放行的唯一情境是「自己送的、而且狀態仍為
`submitted`／`draft`」：離線 outbox 補送時，前一次 POST 可能其實已寫入、只是回應沒收到，
重送必須成功而非報錯。但一旦藥安開始處理，通報者就不能再覆寫——
否則業務按一下重送，就把藥安的判定與編碼整份洗掉。

#### 通報者看得到什麼

業務端有「我的通報紀錄」（唯讀），列出自己送出的個案與目前狀態。
清單由後端過濾，前端沒有、也不需要有「篩掉別人資料」的邏輯——
前端做篩選等於先把別人的病人資料下載到那支手機上再假裝看不到。
...
- 角色分權：業務只讀得到自己送的個案，藥安人員讀寫全部（見 §7A.6）
