...
CIOMS 表格裡「誰通報的」那一段——姓名（26）、員編、電話、信箱、
公司（24a）、轄區——對同一位業務**每次都一樣**。留在通報表單裡，
等於每通報一次就要重打六個欄位，而那正是第一屏，手機上最容易讓人放棄的位置。

改為存在 `ae_users`，首次登入建檔一次，之後表單自動帶入。

#### 為什麼強制建檔、不給跳過

通報驗證要求「可辨識的通報者」（四要素之一）與至少一個聯絡方式。
檔案沒填，第一次通報就會卡在驗證錯誤上——而那時業務人在客戶端、
手上有個真實個案，是**最不該**讓他停下來填基本資料的時刻。
把這件事挪到還沒有時間壓力的第一次登入。

`isProfileComplete()` 只認姓名與電話兩項，正好對齊驗證規則的硬性要求。
門檻訂得比驗證規則高（例如連轄區都要求），只會擋住一個其實可以送出通報的人。

#### 可寫欄位用白名單，不用黑名單

`sanitizeProfile()` 只留下六個白名單欄位，其餘一概丟棄。
用黑名單的話，日後資料表新增敏感欄位而有人忘了加進排除清單，
就會變成**使用者可以自己建檔成藥安人員**。白名單漏掉的後果只是某欄位改不了。

寫入時角色也不在可寫範圍內：既有使用者沿用原角色，新使用者一律建為 `rep`。
目標 email 取自 JWT，請求內容給不了——沒有「幫別人建檔」這條路徑。

#### 表單裡不能直接改

有檔案時，通報表單第一步收成一張摘要卡加一個「編輯」按鈕，六個輸入框不再出現。
不讓表單直接編輯的原因是**兩邊會分岔**：這一份通報寫著新電話、檔案裡還是舊的，
下一份又變回舊的。要改就改檔案，一次改完所有未來的通報。

草稿還原時也一律以檔案覆蓋這幾個欄位：真實來源是檔案，草稿裡的是它的快照。
使用者剛改過檔案卻在草稿看到舊電話，只會讓人以為改了沒生效。

代人通報有專屬位置——「原始通報者」區塊（CIOMS 26 的醫療專業人員），
那才是這個表格裡代人通報的正確欄位。

#### 這不是身分憑證

檔案裡的姓名是**顯示用**資料。「誰送的」永遠是 `ae_cases.submitted_by`，
取自 Access JWT，使用者把姓名改成同事的名字也動不了那個欄位。
建檔功能解決的是「不用重打」，不是「確認身分」——後者早就由 JWT 解決，
而且解得比任何表單欄位都牢靠。
...
- 通報者個人檔案：首次登入建檔一次，通報表單自動帶入（見 §7A.7）
