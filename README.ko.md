...
영업 담당자는 첫 로그인 시 **보고자 정보**(이름, 사번, 전화번호, 회사명 — CIOMS 26／24a)를 한 번만 입력합니다. 이후 모든 보고는 이 프로필에서 자동 입력되어, 폼의 첫 화면이 입력란 6개에서 요약 카드로 바뀝니다. 쓰기 가능한 항목은 명시적 허용 목록이므로, 자신의 프로필을 수정해 PV 담당자로 승격할 수 없습니다.

영업 담당자는 **회사 메일 주소**로 **Cloudflare Access Email OTP**에 로그인합니다——유출되거나 공유되거나 재설정이 필요한 비밀번호가 없으며, 퇴사 시 차단도 자동입니다. 메일함이 비활성화되면 일회용 코드를 받을 수 없으므로 정책을 정리하는 것을 아무도 기억하지 못해도 접근이 끊깁니다.

> ⚠️ 주소는 개별적으로 나열하세요. `Emails ending in @회사도메인` 규칙은 재무·인사·인턴을 포함한 **전 직원**이 환자 이상반응 데이터를 읽을 수 있다는 뜻입니다. 목록이 길어지면 도메인 규칙으로 되돌아가지 말고 Access Group을 사용하세요.
**역할**은 D1에 저장합니다. Access는 "이 사람이 내부인인가"에는 답하지만 "이 사람이 무엇을 봐도 되는가"에는 답하지 않기 때문입니다.

| 역할 | 가능한 작업 |
|---|---|
| `rep`(영업) | 증례 제출, **자신이 제출한 증례만** 조회 |
...
> ⚠️ 시행 지점은 화면이 아니라 Worker입니다. 해시 라우트(`#/report`)는 Access의 경로 규칙으로 분리할 수 없으며——`#` 이후는 서버로 전송되지 않습니다——프런트엔드의 역할 판정은 표시용일 뿐입니다. 각 API가 스스로 역할을 확인하며, 목록 필터링은 JS가 아닌 SQL에서 수행합니다.
