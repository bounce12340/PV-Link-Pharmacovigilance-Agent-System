...
> ⚠️ **역할 분리는 아직 구현되지 않았습니다**. Access를 통과한 사람은 누구나 콘솔을 열고 모든 증례를 볼 수 있습니다. 해시 라우트(`#/report`)는 Access의 경로 규칙으로 분리할 수 없으며——`#` 이후는 서버로 전송되지 않습니다——애플리케이션 계층의 역할 테이블이 필요합니다. 그때까지는 목록의 모든 사람을 PV 담당자로 간주하세요.
